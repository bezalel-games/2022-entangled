...
namespace Tiles
{
    ...
    public class RuleTileWithOther<T> : RuleTileWithOther
    {
        /// <summary>
    ...
    public class RuleTileWithOther : TileBase
    {
        ...
        /// </summary>
        [HideInInspector] public List<TilingRule> m_TilingRules = new List<RuleTileWithOther.TilingRule>();
        ...
        static KeyValuePair<HashSet<TileBase>, HashSet<Vector3Int>> CachingTilemapNeighborPositions(Tilemap tilemap)
        {
            ...
            for (int i = 0; i < usedTileCount; i++)
            {
                ...
                usedTiles.Add(tile);
                RuleTileWithOther ruleTile = null;

                if (tile is RuleTileWithOther rt)
                    ruleTile = rt;
                else if (tile is RuleOverrideTileWithOther ot)
                    ruleTile = ot.m_Tile;
        ...
        public FieldInfo[] GetCustomFields(bool isOverrideInstance)
        {
            return this.GetType().GetFields(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)
                .Where(field => typeof(RuleTileWithOther).GetField(field.Name) == null)
        ...
        {
            if (other is RuleOverrideTileWithOther ot)
